...
export const apiClient = {
  ...
  async listDocuments(docType?: string): Promise<any[]> {
    try {
      const url = docType ? `${API_BASE}/api/documents?doc_type=${encodeURIComponent(docType)}` : `${API_BASE}/api/documents`;
  ...
  async intakeDocument(payload: { title: string; doc_type: string; raw_text: string; sensitivity: number; source_uri?: string }): Promise<any> {
    const res = await fetch(`${API_BASE}/api/documents/intake`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    if (!res.ok) {
      const err = await res.json().catch(() => ({ detail: "Intake failed" }));
  ...
  async getQuarantine(): Promise<any[]> {
    try {
      const res = await fetch(`${API_BASE}/api/documents/quarantine`, {
        headers: { "Content-Type": "application/json" },
        credentials: "include",
      });
      if (res.ok) {
