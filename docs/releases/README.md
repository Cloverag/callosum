...
| `meridian-p1.0.5` | _pending merge_ | 2026-07-28 | [meridian-p1.0.5.md](meridian-p1.0.5.md) — tenancy: migration 0011 puts RLS on the control plane (`membership`, `workspace`) and revokes runtime write grants (incl. `sensitivity`), closing a cross-tenant enumeration path that leaked member names, roles and clearances; `principal` deliberately deferred to CP5 because `membership` is never populated; 160 passed, no code change | |
